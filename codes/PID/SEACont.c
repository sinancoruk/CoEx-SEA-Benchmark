...
int main(int argc, char* argv[]){
  ...
  while(t.tv_sec < LAST_TIME ) {
    ...
    /* Step input*/
    if(((t_currenttime-t_referencetime)/1000000-1)<1){
 		   Reference = 0;
 		 }
 		 else{
       Reference = 5*0.0002;
 ...
 if((time)<0.01)
  ...
  DACWrite();
    ...
    fprintf(fptr,"%f %e %e %e %e %e %e\n", (t_currenttime-t_referencetime)/1000000.-1, SEA.torsion, SEA.motorPos, SEA.torsionVel, SEA.motorVel, Reference, SEA.inputSignal);
