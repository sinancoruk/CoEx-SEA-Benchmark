...
void saturate(void){
  ...
  if(SEA.inputSignal/Kt>9.9999){
    SEA.inputSignal = 9.9999*Kt;
  }
...
void DACWrite(void) {
  ...
  /* This function sends commands to the motor driver via 16-bit DAC */
	// Bu raspi original adaptör için
	// Write_DAC8532(0x30, Voltage_Convert(5.161,(10 + SEA.inputSignal/Kt)/4));
	Write_DAC8532(0x30, Voltage_Convert(5.0,(10 + (SEA.inputSignal)/Kt)/4));
	// Telefon şarj cihazı için
	// Write_DAC8532(0x30, Voltage_Convert(5.3,(10 + SEA.inputSignal/Kt)/4));
...
void getGeneralParameters(void){
	...
	TorsionLowPassFilter.num[0] = 1/(1 + 2/(700*0.0005));
	TorsionLowPassFilter.num[1] = TorsionLowPassFilter.num[0];
	TorsionLowPassFilter.denum[0] = (1 - 2/(700*0.0005)) * TorsionLowPassFilter.num[0];
	TorsionLowPassFilter.output[0] = 0;
	TorsionLowPassFilter.input[0] = 0;
	TorsionLowPassFilter.output[1] = 0;
	TorsionLowPassFilter.input[1] = 0;


	MotorPosDerivative.num[0] = 1/(1 + 2/(500*0.0005));
	MotorPosDerivative.num[1] = MotorPosDerivative.num[0];
	MotorPosDerivative.denum[0] = (1 - 2/(500*0.0005)) * MotorPosDerivative.num[0];
	MotorPosDerivative.output[0] = 0;
	MotorPosDerivative.input[0] = 0;
	MotorPosDerivative.output[1] = 0;
	MotorPosDerivative.input[1] = 0;

	MotorPosDerivativeStiction.num[0] = 1/(1 + 2/(200*0.0005));
	MotorPosDerivativeStiction.num[1] = MotorPosDerivativeStiction.num[0];
	MotorPosDerivativeStiction.denum[0] = (1 - 2/(200*0.0005)) * MotorPosDerivativeStiction.num[0];
	MotorPosDerivativeStiction.output[0] = 0;
	MotorPosDerivativeStiction.input[0] = 0;
	MotorPosDerivativeStiction.output[1] = 0;
	MotorPosDerivativeStiction.input[1] = 0;

	TorsionDerivative.num[0] = 1/(1 + 2/(300*0.0005));
	TorsionDerivative.num[1] = TorsionDerivative.num[0];
	TorsionDerivative.denum[0] = (1 - 2/(300*0.0005)) * TorsionDerivative.num[0];
...
void readEncoders(void) {
  ...
  SEA.torsion = (value2-torsionZero)*enc_constant-SEA.torsionInit;
	...
	SEA.torsionFiltered = TorsionLowPassFilterBlock(SEA.torsion);
  //SEA.motorVel = MotorPosDerivativeBlock(SEA.motorPos*100);

  SEA.motorVel = (SEA.motorPos-SEA.motorPosPrev)*200000;
	SEA.motorPosPrev = SEA.motorPos;

	//SEA.torsionVel = TorsionDerivativeBlock(SEA.torsion);
  SEA.torsionPrev = SEA.torsion;
	//SEA.torsionNm = springK*SEA.torsion;
...
double TorsionLowPassFilterBlock(double inPart){

  /* LPF */
  double TorsionLowPassFilterOut;

  /* Diff equation */
  TorsionLowPassFilter.input[0] = inPart;
  TorsionLowPassFilterOut = TorsionLowPassFilter.num[0] * TorsionLowPassFilter.input[0] + TorsionLowPassFilter.num[1] *  TorsionLowPassFilter.input[1] - TorsionLowPassFilter.denum[0]*TorsionLowPassFilter.output[0];

  /* Update states */
  TorsionLowPassFilter.input[1] =  TorsionLowPassFilter.input[0];
  TorsionLowPassFilter.output[0] = TorsionLowPassFilterOut;

  return TorsionLowPassFilterOut;
}
...
double MotorPosDerivativeBlock(double inPart){
  ...
  /* Diff equation */
  MotorPosDerivative.input[0] = inPart*500;
  ...
  return inPart*500-MotorPosDerivativeOut;
}

double MotorPosDerivativeStictionBlock(double inPart){

	// Input is motor angle (rad) output is motor velocity (rad/s)
  /* LPF */
  double MotorPosDerivativeStictionOut;

  /* Diff equation */
  MotorPosDerivativeStiction.input[0] = inPart*200;
  MotorPosDerivativeStictionOut = MotorPosDerivativeStiction.num[0] * MotorPosDerivativeStiction.input[0] + MotorPosDerivativeStiction.num[1] *  MotorPosDerivativeStiction.input[1] - MotorPosDerivativeStiction.denum[0]*MotorPosDerivativeStiction.output[0];

  /* Update states */
  MotorPosDerivativeStiction.input[1] =  MotorPosDerivativeStiction.input[0];
  MotorPosDerivativeStiction.output[0] = MotorPosDerivativeStictionOut;

  return inPart*200-MotorPosDerivativeStictionOut;
}
...
double TorsionDerivativeBlock(double inPart){
  ...
  /* Diff equation */
  TorsionDerivative.input[0] = inPart*300;
  ...
  return inPart*300-TorsionDerivativeOut;
}
